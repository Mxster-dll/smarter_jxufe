...
///
/// - 形状 = 圆角 12 + `outlineVariant` 60% 淡边框（无阴影）
/// - 底色 = 纯白（与 `lib/main.dart` 主题「纯白」口径一致）
/// - 内边距 = 14
/// - 强调色 = `colorScheme.primary`（校红 `#C3282E`，用户裁定：单色强调一律改红）
///
...
/// 卡片底色（纯白；不用 `colorScheme.surfaceContainerLow` —— 那是 #FAFAFA）。
const Color kAppCardColor = Color(0xFFFFFFFF);

/// 卡片描边：`outlineVariant` 的 60% 透明度（目标样式的原始取值）。
BorderSide appCardBorderSide(ColorScheme scheme) =>
...
/// 卡片强调色的淡底（图标底板等，alpha 0.10 与首页磁贴口径一致）。
Color appCardAccentSoft(BuildContext context) =>
    appCardAccent(context).withValues(alpha: 0.10);
...
///
/// 需要点击/涟漪时用 `Material(color: kAppCardColor, shape: appCardShape(context))`，
/// 而不是本函数。
Widget appCard(
