...
///    各一张进度卡（实际 / 平台两档口径）；后三者点击进平台明细表；
///    入馆教育那张点击进入原「新生入馆教育」页（App 内闯关）。
/// 2. 「学分说明」= 平台规则原文（重排版）。
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          ref.invalidate(jhReadRecordsProvider);
        },
        child: ListView(
    ...
    return _card(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.info_outline, size: 17, color: FeaturePalette.jhRead),
