...
enum CourseAttempt {
  ...
  factory CourseAttempt.parse(String source) => switch (source) {
    '初修' => .first,
    '重修' => .retake,
...
class CourseGrade {
  ...
  CourseGrade(
    ...
    double? credit,
    double? gradePoint,
    double? gradePointCredit,
    this.attempt = .first,
    this.remark = '',
  }) {
    credit = credit ?? (score.hasPassed ? course.credit : 0);
    gradePoint = gradePoint ?? (score.hasPassed ? course.credit / 10 - 5 : 0);
    gradePointCredit = gradePointCredit ?? (credit * gradePoint);
  }
}
